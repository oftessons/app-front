...
export class ComentariosAulasService {

  private apiURL: string = environment.apiURLBase + '/api/aula/comentarios';
  ...
  obterComentarios(aulaId: number, cursor?: string, limit: number = 10): Observable<CursorPageResponse<ComentarioAulaResponse>> {
    ...
    return this.http.get<CursorPageResponse<ComentarioAulaResponse>>(`${this.apiURL}/${aulaId}`, { params }).pipe(
      catchError(this.handleError)
    );
  }
   ...
   */
  atualizarComentario(aulaId: number, comentarioId: number, request: ComentarioAulaRequest): Observable<void> {
    return this.http.put<void>(`${this.apiURL}/${aulaId}/${comentarioId}`, request).pipe(
      catchError(this.handleError)
    );
  }
   ...
   */
  deletarComentario(aulaId: number, comentarioId: number): Observable<void> {
  ...
  /**
   * Edita uma resposta de comentário
   */
  editarResposta(aulaId: number, comentarioId: number, respostaId: number, request: RespostaComentarioRequest): Observable<void> {
    return this.http.put<void>(`${this.apiURL}/${aulaId}/${comentarioId}/respostas/${respostaId}`, request).pipe(
      catchError(this.handleError)
    );
  }

  /**
   * Deleta uma resposta de comentário
   */
  deletarResposta(aulaId: number, comentarioId: number, respostaId: number): Observable<void> {
    return this.http.delete<void>(`${this.apiURL}/${aulaId}/${comentarioId}/respostas/${respostaId}`).pipe(
      catchError(this.handleError)
    );
  }
